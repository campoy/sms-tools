...
def testRealEven(x):
    ...
    dftbuffer[M-M/2:] = x[:M/2]
    dftbuffer[:1+M/2] = x[M/2:]

    X = fft(dftbuffer)
    positive = X[1:M/2+1]
    negative = X[M/2+1:]
    is_real = all(np.real(positive) == np.real(negative[::-1]))
    is_even = all(abs(np.imag(X)) < 10e-10)
    return is_real and is_even, dftbuffer, X
