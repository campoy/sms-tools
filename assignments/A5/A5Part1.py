...
def minFreqEstErr(inputFile, f):
    ...
    while True:
        ...
        x1 = x[time-hM:time+hM+1]

        w = get_window(window, M)
        mX, pX = DFT.dftAnal(x1, w, N)
    
        plocs = UF.peakDetection(mX, t)
        iploc, ipmag, ipphase = UF.peakInterp(mX, pX, plocs)
        iploc = iploc*fs/float(N)
        ...
        err =  abs(iploc[0]-f)
        if err < 0.05:
            # plt.plot(np.arange(0, fs/2, fs/float(N)), mX[:-1])
            # plt.plot(iploc, ipmag, 'ro')
            # plt.text(iploc+10*fs/float(N), ipmag, r'%f'%iploc[0])
            # plt.axis([0, 2000, t, 0])
            # plt.show()                
            return iploc[0], M, N